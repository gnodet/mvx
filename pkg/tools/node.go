...
func (n *NodeTool) Verify(version string, cfg config.ToolConfig) error {
	...
	if err != nil { return fmt.Errorf("node verification failed: %w\nOutput: %s", err, out) }
...
func (n *NodeTool) getDownloadURL(version string) string {
	...
	switch runtime.GOOS {
	case "linux":
		if runtime.GOARCH == "arm64" { platform = "linux-arm64" } else { platform = "linux-x64" }
	case "darwin":
		if runtime.GOARCH == "arm64" { platform = "darwin-arm64" } else { platform = "darwin-x64" }
	case "windows":
		platform = "win-x64"
	}
...
func (n *NodeTool) downloadAndExtract(url, destDir string) error {
	...
	if resp.StatusCode != 200 { return fmt.Errorf("download failed: %d from %s", resp.StatusCode, url) }
	if _, err := io.Copy(tmp, resp.Body); err != nil { return err }
	_ = tmp.Close()

	if strings.HasSuffix(url, ".zip") { return extractZipFile(tmp.Name(), destDir) }
...
func extractZipFile(src, dest string) error {
	...
	for _, f := range r.File {
		...
		if f.FileInfo().IsDir() {
			if err := os.MkdirAll(path, f.Mode()); err != nil { return err }
			continue
		}
		if err := os.MkdirAll(filepath.Dir(path), 0755); err != nil { return err }
		rc, err := f.Open(); if err != nil { return err }
		out, err := os.OpenFile(path, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, f.Mode()); if err != nil { rc.Close(); return err }
