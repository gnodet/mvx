...
func ResolvePlatformScriptWithInterpreter(script interface{}, defaultInterpreter string) (string, string, error) {
	switch s := script.(type) {
	case string:
		return s, defaultInterpreter, nil // Simple string script with default interpreter
	case map[string]interface{}:
		platform := runtime.GOOS
		...
		if str, ok := platformValue.(string); ok {
			return str, defaultInterpreter, nil
	...
	case PlatformScript:
		...
		return resolvedScript, defaultInterpreter, err
	default:
		return "", "", fmt.Errorf("invalid script type: %T", script)
	}
}
